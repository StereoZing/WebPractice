...
if (strlen($_POST['name1'])>150) {
	setcookie('name1_error', 'ФИО слишком длинное.');
	$error = TRUE;
} elseif (preg_match('~[0-9]+~', $_POST['name1'])) {
	setcookie('name1_error', 'ФИО не должно содержать цифры.');
	$error = TRUE;
} else {
	setcookie('name1_error', '', strtotime("-1 day"));
}
setcookie('name1_value', $_POST['name1'], strtotime('+1 year'));
...
if($year<1800){
	setcookie('date1_error', 'Вы долгожитель?');
	$error = TRUE;
} elseif ($year>2025) {
	setcookie('date1_error', 'Вы из будущего?');
	$error = TRUE;
} else {
	setcookie('date1_error', '', strtotime("-1 day"));
}
setcookie('date1_value', $_POST['date1'], strtotime('+1 year'));

if (empty($_POST['languages'])){
	setcookie('languages_error', 'Выберите хотя бы JavaScript');
	setcookie('languages_value', '', strtotime('-1 day'));
	$error = TRUE;
} else {
	setcookie('languages_error', '', strtotime("-1 day"));
	setcookie('languages_value', implode('|', ($_POST['languages'])), strtotime('+1 year'));
}

if (!preg_match('/^[0, 1]$/', $_POST['pol'])){
	setcookie('pol_error',"Ты как это сделал?");
	$error=TRUE;
} else {
	setcookie('pol_error', '', strtotime("-1 day"));
}
setcookie('pol_value', $_POST['pol'], strtotime('+1 year'));

if (strlen($_POST['biography'])>200){
	setcookie('biography_error', 'Ваша биография трогает душу...<br/>Но уменьшите её (душу)');
	$error = TRUE;
} else {
	setcookie('biography_error', '', strtotime("-1 day"));
}
setcookie('biography_value', $_POST['biography'], strtotime('+1 year'));
...
try {
	...
	foreach($_POST['languages'] as $value) {
		$stmt = $db->prepare("INSERT INTO app_languagess VALUES (?, ?)");
...
<html languages="ru">
  ...
  <head>
    ...
    <link
      ...
      integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmdate15pNlyT2bRjXh0JMhjY6hW+ALEwIH"
      ...
      <section class="content order-3 order-sm-3 mt-3 p-3" id="form">
        ...
        <form action="./" method="POST">
            ...
            <label>
                ...
                <select name="languagesuages[]" multiple="multiple">
            ...
            <label>
                Биография:<br />
                <textarea name="biographygraphy"></textarea>
            </label><br />
